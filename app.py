...
if password != "3811":
    if password:
        st.error("암호가 틀렸습니다.")
    st.info("이 프로그램은 허가된 사용자만 이용할 수 있습니다.")
    st.stop()
...
st.caption("엄마 말의 이름을 입력하면, 연결된 모든 자식들이 검색됩니다.")
...
if search_keyword:
    ...
    if not found_mom:
        st.warning(f"❌ '{search_keyword}' 이름으로 등록된 종빈마 데이터가 없습니다.")

# --- [주요 기능 2: 씨수말 랭킹] ---
st.divider()
st.markdown("### 📊 연도별 씨수말 배출 현황")
...
if sorted_results:
    st.info(f"✅ 검색 기간 내 총 {len(sorted_results)}두의 씨수말이 검색되었습니다.")
    for i, (sire_name, daughters, total_count) in enumerate(sorted_results[:50], 1):
        stars = "⭐" * min(len(daughters), 10)
        with st.expander(f"[{i}위] {sire_name} (기간 내: {len(daughters)}두) {stars}"):
            for d in daughters:
