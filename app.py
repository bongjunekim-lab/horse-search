...
@st.cache_data
def load_data():
    # 파일명은 선생님이 올리신 파일명과 정확히 일치해야 합니다.
    ...
    def traverse(node, parent_text="Unknown"):
        ...
        # [가지연결] 현재 노드 바로 아래 자식(선으로 연결된) 텍스트 수집
        direct_children = []
        for child in node:
...
st.caption("요청하신 대로 검색창을 없애고 랭킹을 가장 먼저 보여줍니다.")
...
# 화면에 랭킹 리스트 출력
if not sorted_list:
    st.warning("데이터를 분석 중이거나 조건에 맞는 말이 없습니다.")
else:
    st.success(f"✅ 총 {len(sorted_list)}두의 씨수말 랭킹이 검색되었습니다.")
    
    for i, (sire, daughters, total) in enumerate(sorted_list[:50], 1):
        # 체크박스(0.05) + 정보(0.95)
        c1, c2 = st.columns([0.05, 0.95])
        
        # 2차 결과 확인을 위한 고유 체크박스
        show_detail = c1.checkbox("", key=f"rank_item_{i}")
        
        with c2:
            stars = "⭐" * min(len(daughters), 10)
            st.markdown(f"**[{i}위] {sire}** (기간 내 @: {len(daughters)}두 / 전체 @: {total}두) {stars}")
            
            # 체크박스 클릭 시 '선으로 연결된' 자마만 상세 분석 (2차 결과)
            if show_detail:
                with st.container(border=True):
                    st.write(f"📂 **{sire}** 배출 엘리트 자마(@)의 하부 선 연결 정보")
                    for elite_mare in daughters:
                        # branch_map에서 '가지'로 연결된 자식들만 발췌
